...
namespace BitBenderGames {
  ...
  public class MobileTouchCamera : MonoBehaviourWrapped {
        ...
        public static MobileTouchCamera instance;
    ...
    private float minOvertiltSpringPositionThreshold = 0.1f; //This value is necessary to reposition the camera and do boundary update computations while the auto spring back from overtilt is active and larger than this value.
        ...
        void OnEvBuildMode(EventData data)
        {
            ...
            if (isBuildMode == true)
            {
                ...
                //transform.DOMove(new Vector3(0, 19, 0),0.5f);
                transform.DOLocalRotate(new Vector3(90, 0, 0), 0.5f).OnComplete(()=> { enabled = true; });
            ...
            else
            {
                ...
                //transform.DOMove(new Vector3(0, 10, -20), 0.5f);
                transform.DOLocalRotate(new Vector3(40, 0, 0), 0.5f).OnComplete(() => { enabled = true; });
            }
        }

        void OnEvCameraDrag(EventData data)
        {
            var exdata = data as EventDataEx<bool>;
            bool isDrag = exdata.GetData();
            isDraggingSceneObject = !isDrag;
        }
        void OnEvBuildCamMove(EventData data)
        {
        ...
        {
            EventManager.Instance.AddEventListener(Common.EventStr.BuildMode, OnEvBuildMode);
            EventManager.Instance.AddEventListener(Common.EventStr.BuildCamMove, OnEvBuildCamMove);
            EventManager.Instance.AddEventListener(Common.EventStr.CameraDrag, OnEvCameraDrag);
            
        }

        private void OnDisable()
        {
            EventManager.Instance.RemoveEventListener(Common.EventStr.BuildMode, OnEvBuildMode);
            EventManager.Instance.RemoveEventListener(Common.EventStr.BuildCamMove, OnEvBuildCamMove);
            EventManager.Instance.RemoveEventListener(Common.EventStr.CameraDrag, OnEvCameraDrag);
        }
