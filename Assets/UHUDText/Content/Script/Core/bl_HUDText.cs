...
public class bl_HUDText : MonoBehaviour {
   ...
   [SerializeField] private List<bl_Text> texts = new List<bl_Text>();
    ...
    public void RemoveText(Transform trans)
    {
        for (int i = texts.Count-1; i >= 0; i--)
        {
            if (texts[i].transform == trans)
            {
                texts.RemoveAt(i);
                break;
            }
    ...
    void HudTextControl()
    {
        for (int i = 0; i < texts.Count; i++)
        {
            ...
            //else//if UI visible
            {
                ...
                {
                    //position = temporal.m_Transform.Find("LookAt").position;
                ...
                Vector3 front = position - m_Cam.transform.position;
                //its in camera view
                if ((front.magnitude <= HideDistance) && (Vector3.Angle(m_Cam.transform.forward, position - m_Cam.transform.position) <= MaxViewAngle))
                {
                    //Convert position to view port
                    Vector2 v = m_Cam.WorldToViewportPoint(position);
                    //Calculate font size depend of distance.
                    //int fontSize = ((int)((mov / 4) * 1) + temporal.m_Size) / 2;
                    int fontSize = ((int)((mov / 4) * 1) + temporal.m_Size) / 3;
    ...
    public GameObject CreateNewText(bl_Text item,HUDTextInfo info)
    {
        //override animation type
        TextAnimationType tat = (info.AnimationType == TextAnimationType.None) ? m_TextAnimationType : info.AnimationType;
        GameObject prefab = (info.TextPrefab == null) ? TextPrefab : info.TextPrefab;
    ...
    public GameObject CreateNewText(bl_Text item, HUDTextInfo info,string type)
    {
        //override animation type
        TextAnimationType tat = (info.AnimationType == TextAnimationType.None) ? m_TextAnimationType : info.AnimationType;

        GameObject prefab = info.TextPrefab;
        //Create new text info to instantiate 
    ...
    public GameObject UpdateImageText(bl_Text item, HUDTextInfo info, string type)
    {
        TextAnimationType tat = (info.AnimationType == TextAnimationType.None) ? m_TextAnimationType : info.AnimationType;
        item.m_Speed = info.Speed;
        item.FadeSpeed = (info.FadeSpeed > 0) ? info.FadeSpeed : FadeSpeed;
    ...
    private int ScreenPosition(Transform t)
    {
        ...
        if (m_Type == GameType.Mode3D)
        {
            p = (int)(m_Cam.WorldToScreenPoint(t.GetComponent<Collider>().bounds.center + (((Vector3.up * t.GetComponent<Collider>().bounds.size.y) * 0.5f))).y - 
            ...
            //p = (int)(m_Cam.WorldToScreenPoint(t.position);
        }
