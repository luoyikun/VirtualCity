...
namespace Org.BouncyCastle.Math.EC
{
    public class ECAlgorithms
    {
        ...
        {
            return field.Dimension > 1 && field.Characteristic.Equals(BigIntegerHttp.Two)
        ...
        public static ECPoint SumOfMultiplies(ECPoint[] ps, BigIntegerHttp[] ks)
        {
        ...
        public static ECPoint SumOfTwoMultiplies(ECPoint P, BigIntegerHttp a, ECPoint Q, BigIntegerHttp b)
        {
            ECCurve cp = P.Curve;
            Q = ImportPoint(cp, Q);
            ...
            if (glvEndomorphism != null)
            {
                return ValidatePoint(
                    ImplSumOfMultipliesGlv(new ECPoint[] { P, Q }, new BigIntegerHttp[] { a, b }, glvEndomorphism));
            }
        ...
        * 9: return R
        */
        public static ECPoint ShamirsTrick(ECPoint P, BigIntegerHttp k, ECPoint Q, BigIntegerHttp l)
        {
            ECCurve cp = P.Curve;
            Q = ImportPoint(cp, Q);
         ...
         */
        public static ECPoint ReferenceMultiply(ECPoint p, BigIntegerHttp k)
        {
            BigIntegerHttp x = k.Abs();
        ...
        internal static ECPoint ImplShamirsTrickJsf(ECPoint P, BigIntegerHttp k, ECPoint Q, BigIntegerHttp l)
        {
        ...
        internal static ECPoint ImplShamirsTrickWNaf(ECPoint P, BigIntegerHttp k,
            ECPoint Q, BigIntegerHttp l)
        {
        ...
        internal static ECPoint ImplShamirsTrickWNaf(ECPoint P, BigIntegerHttp k, ECPointMap pointMapQ, BigIntegerHttp l)
        {
            bool negK = k.SignValue < 0, negL = l.SignValue < 0;
        ...
        internal static ECPoint ImplSumOfMultiplies(ECPoint[] ps, BigIntegerHttp[] ks)
        {
            int count = ps.Length;
            ...
            {
                BigIntegerHttp ki = ks[i]; negs[i] = ki.SignValue < 0; ki = ki.Abs();
        ...
        internal static ECPoint ImplSumOfMultipliesGlv(ECPoint[] ps, BigIntegerHttp[] ks, GlvEndomorphism glvEndomorphism)
        {
            BigIntegerHttp n = ps[0].Curve.Order;

            int len = ps.Length;

            BigIntegerHttp[] abs = new BigIntegerHttp[len << 1];
            for (int i = 0, j = 0; i < len; ++i)
            {
                BigIntegerHttp[] ab = glvEndomorphism.DecomposeScalar(ks[i].Mod(n));
                abs[j++] = ab[0];
                abs[j++] = ab[1];
            }
        ...
        internal static ECPoint ImplSumOfMultiplies(ECPoint[] ps, ECPointMap pointMap, BigIntegerHttp[] ks)
        {
            ...
            for (int i = 0; i < halfCount; ++i)
            {
                ...
                BigIntegerHttp kj0 = ks[j0]; negs[j0] = kj0.SignValue < 0; kj0 = kj0.Abs();
                BigIntegerHttp kj1 = ks[j1]; negs[j1] = kj1.SignValue < 0; kj1 = kj1.Abs();
