		...
		: IAsymmetricBlockCipher
	{
		...
		*/
			...
			int		length)
		{
			...
			BigIntegerHttp p = key.Parameters.P;

			byte[] output;
			if (key is ElGamalPrivateKeyParameters) // decryption
			{
				int halfLength = length / 2;
				BigIntegerHttp gamma = new BigIntegerHttp(1, input, inOff, halfLength);
				BigIntegerHttp phi = new BigIntegerHttp(1, input, inOff + halfLength, halfLength);

				ElGamalPrivateKeyParameters priv = (ElGamalPrivateKeyParameters) key;

				// a shortcut, which generally relies on p being prime amongst other things.
				// if a problem with this shows up, check the p and g values!
				BigIntegerHttp m = gamma.ModPow(p.Subtract(BigIntegerHttp.One).Subtract(priv.X), p).Multiply(phi).Mod(p);

				output = m.ToByteArrayUnsigned();
			}
			else // encryption
			{
				BigIntegerHttp tmp = new BigIntegerHttp(1, input, inOff, length);
				...
				BigIntegerHttp pSub2 = p.Subtract(BigIntegerHttp.Two);

				// TODO In theory, a series of 'k', 'g.ModPow(k, p)' and 'y.ModPow(k, p)' can be pre-calculated
				BigIntegerHttp k;
				do
				{
					k = new BigIntegerHttp(p.BitLength, random);
				}
				while (k.SignValue == 0 || k.CompareTo(pSub2) > 0);

				BigIntegerHttp g = key.Parameters.G;
				BigIntegerHttp gamma = g.ModPow(k, p);
				BigIntegerHttp phi = tmp.Multiply(pub.Y.ModPow(k, p)).Mod(p);
