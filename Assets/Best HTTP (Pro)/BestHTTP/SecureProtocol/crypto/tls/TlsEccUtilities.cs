...
namespace Org.BouncyCastle.Crypto.Tls
{
    public abstract class TlsEccUtilities
    {
        ...
        public static byte[] SerializeECFieldElement(int fieldSize, BigIntegerHttp x)
        {
        ...
        public static BigIntegerHttp DeserializeECFieldElement(int fieldSize, byte[] encoding)
        {
            int requiredLength = (fieldSize + 7) / 8;
            if (encoding.Length != requiredLength)
                throw new TlsFatalAlert(AlertDescription.decode_error);
            return new BigIntegerHttp(1, encoding);
        }
        ...
        public static byte[] CalculateECDHBasicAgreement(ECPublicKeyParameters publicKey, ECPrivateKeyParameters privateKey)
        {
            ...
            BigIntegerHttp agreementValue = basicAgreement.CalculateAgreement(publicKey);
        ...
        public static AsymmetricCipherKeyPairHttp GenerateECKeyPair(SecureRandom random, ECDomainParameters ecParams)
        {
        ...
        {
            AsymmetricCipherKeyPairHttp kp = GenerateECKeyPair(random, ecParams);
        ...
        {
            BigIntegerHttp K = ReadECParameter(input);
            if (K.BitLength < 32)
            {
                int k = K.IntValue;
                if (k > 0 && k < fieldSize)
        ...
        public static BigIntegerHttp ReadECFieldElement(int fieldSize, Stream input)
        {
            return DeserializeECFieldElement(fieldSize, TlsUtilities.ReadOpaque8(input));
        }

        public static BigIntegerHttp ReadECParameter(Stream input)
        {
            // TODO Are leading zeroes okay here?
            return new BigIntegerHttp(1, TlsUtilities.ReadOpaque8(input));
        }
        ...
        public static ECDomainParameters ReadECParameters(int[] namedCurves, byte[] ecPointFormats, Stream input)
        {
            try
            {
                ...
                case ECCurveType.explicit_prime:
                {
                    ...
                    BigIntegerHttp prime_p = ReadECParameter(input);
                    BigIntegerHttp a = ReadECFieldElement(prime_p.BitLength, input);
                    BigIntegerHttp b = ReadECFieldElement(prime_p.BitLength, input);
                    byte[] baseEncoding = TlsUtilities.ReadOpaque8(input);
                    BigIntegerHttp order = ReadECParameter(input);
                    BigIntegerHttp cofactor = ReadECParameter(input);
                ...
                case ECCurveType.explicit_char2:
                {
                    ...
                    BigIntegerHttp a = ReadECFieldElement(m, input);
                    BigIntegerHttp b = ReadECFieldElement(m, input);
                    byte[] baseEncoding = TlsUtilities.ReadOpaque8(input);
                    BigIntegerHttp order = ReadECParameter(input);
                    BigIntegerHttp cofactor = ReadECParameter(input);
        ...
        {
            BigIntegerHttp K = BigIntegerHttp.ValueOf(k);
        ...
        public static void WriteECFieldElement(int fieldSize, BigIntegerHttp x, Stream output)
        {
            TlsUtilities.WriteOpaque8(SerializeECFieldElement(fieldSize, x), output);
        }

        public static void WriteECParameter(BigIntegerHttp x, Stream output)
        {
