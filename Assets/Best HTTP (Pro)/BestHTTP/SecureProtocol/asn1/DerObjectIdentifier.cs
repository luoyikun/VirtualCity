...
namespace Org.BouncyCastle.Asn1
{
        ...
        : Asn1Object
    {
            ...
            BigIntegerHttp	fieldValue)
        {
            ...
            {
                BigIntegerHttp tmpValue = fieldValue;
        ...
        private void DoOutput(MemoryStream bOut)
        {
            ...
            {
                WriteField(bOut, new BigIntegerHttp(token).Add(BigIntegerHttp.ValueOf(first)));
            }
            ...
            while (tok.HasMoreTokens)
            {
                ...
                {
                    WriteField(bOut, new BigIntegerHttp(token));
                }
            }
        }
            ...
            byte[] bytes)
        {
            ...
            for (int i = 0; i != bytes.Length; i++)
            {
                ...
                else
                {
                    if (bigValue == null)
                    {
                        bigValue = BigIntegerHttp.ValueOf(value);
                    }
                    bigValue = bigValue.Or(BigIntegerHttp.ValueOf(b & 0x7f));
                    if ((b & 0x80) == 0)
                    {
                        if (first)
                        {
                            objId.Append('2');
                            bigValue = bigValue.Subtract(BigIntegerHttp.ValueOf(80));
                            first = false;
                        }
