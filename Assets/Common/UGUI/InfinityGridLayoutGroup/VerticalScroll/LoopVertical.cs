...
public class LoopVertical : MonoBehaviour
{
        ...
        if (!hasInit)
        {
            ...
            if (m_isSelect == true)
            {
                for (int i = 0; i < transform.childCount; i++)
                {
                    ...
                    trans.gameObject.AddComponent<LoopItem>();
                    ClickListener.Get(trans.gameObject).onClick = OnBtnItem;
                }
            }
        }
        else
    ...
    void OnBtnItem(GameObject obj)
    {
        ...
        LoopItem item = obj.GetComponent<LoopItem>();
        if (m_selectIdx == item.m_idx)
        {
            return;
        }
        for (int i = 0; i < transform.childCount; i++)
        {
            ...
            {
                if (trans.GetComponent<LoopItem>().m_onUnselect != null)
                {
                    trans.GetComponent<LoopItem>().m_onUnselect(trans.gameObject);
                }
            }
        }

        m_selectIdx = item.m_idx;
    ...
    public void SetAmount(int count)
    {
        ...
        //if (amount < minAmount)
        //{
        //    m_tarns.parent.GetComponent<ScrollRect>
        //}
        InitChildren();
        //StartCoroutine(InitChildren());
    }
