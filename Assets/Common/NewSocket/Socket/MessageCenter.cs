    ...
    void Update()
    {
        ...
        while (_netMessageDataQueue.Count > 0)
        {
            lock (_netMessageDataQueue)
            {
                ...
                if (SpProto.IsRecvContain(tmpNetMessageData.m_key))
                {
                    tmpNetMessageData._eventData = UnityGZip.DeCompress(tmpNetMessageData._eventData);
                }


                if (SpProto.IsCancelLoadContain(tmpNetMessageData.m_key))
                {
                    //if (UIManager.Instance.IsTopPanel(Vc.AbName.uiloadpanel))
                    //{
