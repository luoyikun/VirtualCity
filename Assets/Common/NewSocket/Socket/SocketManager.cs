...
//#endif
        ...
        public void SendMsgProtoVoid(string msgId, EnSocket type = EnSocket.Game)
        {
        ...
        public void SendMsgProto<T>(string msgId, T content, EnSocket type = EnSocket.Game)
        {
            ...
            if (clientSocket == null || clientSocket.Connected == false || _isConnected == false && msgId != MsgIdDefine.ReqHeartBeatMessage)
            {
            ...
            if (SpProto.IsNeedLoadContain(msgId))
            {
                //UIManager.Instance.PushPanel(Vc.AbName.uiloadpanel, false, false, null, true); 
